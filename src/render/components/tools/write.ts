...
import { formatBytes, inferCodeLanguage, previewCodeBlock, renderFileChanges, renderToolCard, stringProp } from './shared';
...
export function renderWriteTool(entry: ToolHistoryEntry, ctx: RenderContext) {
  ...
  const bodyBlock = [
    ...(!entry.fileChanges?.length && content ? previewCodeBlock(content, inferCodeLanguage(path), ctx, 8) : []),
