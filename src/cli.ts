...
function printHelp() {
  const sections = [
    ...
    formatRows([
      ...
      ['/about', 'Show build, system, and account info'],
      ['/ask', 'Toggle read-only ask mode'],
      ['/btw <question>', 'Ask a side question without changing chat context'],
      ['/commit', 'Ask the agent to stage and commit changes'],
      ['/copy-conversation-id', 'Copy the current conversation ID'],
      ['/copy-request-id', 'Copy the last model request ID'],
      ['/logout', 'Log out of Cue'],
      ['/model', 'Switch models'],
      ['/plan [prompt]', 'Queue a planning turn or toggle planning mode'],
      ...
      ['/shell [command]', 'Enter shell mode or run a shell command'],
      ['/show-thinking', 'Toggle reasoning block display'],
      ['/simplify', 'Run a read-only simplification review'],
