...
export const OPENAI_MODEL_OPTIONS: OpenAIModelOption[] = [
  ...
  },
  {
    id: 'gpt-5.4-nano',
    label: 'GPT-5.4 nano',
    description: 'Smallest GPT-5.4 variant for speed and cost',
    contextWindow: 400_000,
    reasoning: true,
  },
  {
    id: 'gpt-5.2',
    label: 'GPT-5.2',
    description: 'Balanced GPT-5 generation with strong reasoning',
    contextWindow: 400_000,
    reasoning: true,
  },
  {
    id: 'gpt-5.1',
    label: 'GPT-5.1',
    description: 'Earlier GPT-5 release with broad reasoning support',
    contextWindow: 400_000,
    reasoning: true,
  },
  {
    id: 'gpt-5.1-codex',
    label: 'GPT-5.1 Codex',
    description: 'GPT-5.1 tuned toward code-heavy workflows',
    contextWindow: 400_000,
    reasoning: true,
  },
  {
  ...
  },
  {
    id: 'gpt-5',
    label: 'GPT-5',
    description: 'General GPT-5 frontier model',
    contextWindow: 400_000,
    reasoning: true,
  },
  {
    id: 'gpt-5-mini',
    label: 'GPT-5 mini',
    description: 'Smaller GPT-5 tuned for speed and cost',
    contextWindow: 400_000,
    reasoning: true,
  },
  {
    id: 'gpt-5-nano',
    label: 'GPT-5 nano',
    description: 'Tiny GPT-5 for lightweight quick tasks',
    contextWindow: 400_000,
    reasoning: true,
  },
  {
    id: 'gpt-4.1',
    label: 'GPT-4.1',
    description: 'Reliable general model with a huge context window',
    contextWindow: 1_000_000,
  },
  {
    id: 'gpt-4.1-mini',
    label: 'GPT-4.1 mini',
    description: 'Fast low-cost GPT-4.1 for common work',
    contextWindow: 1_000_000,
  },
  {
    id: 'gpt-4.1-nano',
  ...
  },
  {
    id: 'gpt-4o',
    label: 'GPT-4o',
    description: 'Realtime-friendly all-rounder',
    contextWindow: 128_000,
  },
  {
    id: 'gpt-4o-mini',
    label: 'GPT-4o mini',
    description: 'Cheap fast GPT-4o for lighter tasks',
    contextWindow: 128_000,
  },
  {
    id: 'o1',
    label: 'o1',
    description: 'Deliberate reasoning model for harder problems',
    contextWindow: 128_000,
    reasoning: true,
  },
  {
    id: 'o1-mini',
    label: 'o1 mini',
    description: 'Cheaper smaller o1 reasoning model',
    contextWindow: 128_000,
    reasoning: true,
  },
  {
    id: 'o1-pro',
    label: 'o1 pro',
    description: 'Higher-end o1 for deeper reasoning',
  ...
  },
  {
    id: 'o3',
    label: 'o3',
    description: 'Top-end reasoning model for hard analysis and coding',
    contextWindow: 200_000,
    reasoning: true,
  },
  {
    id: 'o3-mini',
    label: 'o3 mini',
    description: 'Cheaper STEM and coding-focused reasoning model',
    contextWindow: 200_000,
    reasoning: true,
  },
  {
  ...
  },
  {
    id: 'codex-mini',
    label: 'Codex Mini',
    description: 'Stable Codex Mini alias for coding-focused work',
    contextWindow: 200_000,
    reasoning: true,
  },
];
...
const MODEL_OPTION_MAP = new Map(
  OPENAI_MODEL_OPTIONS.map(option => [option.id.toLowerCase(), option]),
);
...
function inferContextWindow(model: string) {
  ...
  if (normalized.startsWith('o3') || normalized.startsWith('o4') || normalized.includes('codex'))
    return 200_000;
...
export function getOpenAIModelDescription(model: string) {
  return (
    getKnownOpenAIModel(model)?.description ??
    getOpenAIPricingMeta(model)?.model?.description ??
    null
  );
}
...
export function getOpenAIContextWindow(model: string) {
  ...
  return (
    ...
    getKnownOpenAIModel(model)?.contextWindow ??
...
export function getOpenAIModelPricingSummary(model: string) {
  ...
  const parts = [
    ...
    cacheRead ? `${cacheRead} cache` : null,
  ].filter(Boolean);
...
export function cycleThinkingMode(current: ThinkingMode, model: string) {
  ...
  return (
    supportedModes[(index + 1 + supportedModes.length) % supportedModes.length] ?? supportedModes[0]
  );
}
...
export function createOpenAIProviderOptions(
  ...
  options: { includeReasoningSummary?: boolean } = {},
) {
