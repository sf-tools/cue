...
async function loadNotebook(path: string): Promise<{ raw: string; notebook: Notebook }> {
  ...
  try {
    ...
    throw new Error(`failed to parse notebook ${path}: ${error instanceof Error ? error.message : String(error)}`);
  }
...
export function createNotebookReadTool() {
  return tool({
    description: 'List the cells of a Jupyter notebook (.ipynb): cell index, type, source preview, and a summary of any cached outputs.',
    ...
    execute: async ({ path, max_cells }) => {
      ...
      const lines: string[] = [`${path} · ${notebook.cells.length} cell${notebook.cells.length === 1 ? '' : 's'}`];

      cells.forEach((cell, index) => {
        const source = readSource(cell);
        const summary = describeOutput(cell.outputs);
        const exec = typeof cell.execution_count === 'number' ? ` exec ${cell.execution_count}` : '';
  ...
  return tool({
    ...
    inputSchema: z.discriminatedUnion('action', [
      ...
      z.object({
        ...
        path: z.string().min(1),
        index: z.number().int().nonnegative().describe('Insert position (use cells.length to append).'),
        source: z.string(),
      ...
      z.object({
        ...
        path: z.string().min(1),
        index: z.number().int().nonnegative().optional().describe('If omitted, clears outputs of every code cell.')
      })
    ]),
    execute: async input => {
      ...
      if (input.action === 'set') {
        if (input.index >= notebook.cells.length) throw new Error(`cell index ${input.index} out of range (have ${notebook.cells.length})`);
        ...
        const targets = input.index === undefined ? notebook.cells.map((_, idx) => idx) : [input.index];
        for (const idx of targets) {
          if (idx >= notebook.cells.length) throw new Error(`cell index ${idx} out of range (have ${notebook.cells.length})`);
